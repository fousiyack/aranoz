...
def register(request):
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            #Check your email for further instruction, don't forget to check SPAM folder too.
            ...
            send_email.send()
            #messages.success(request,'Thank you for registering.We have sent you verification to your email.pls verify it')
...
def activate(request,uidb64, token):
    try:  
        # Encode the user's primary key as a base64 string
    ...
    if request.method=="POST":
        ...
        if password == confirm_password:
            ...
            if (len(password) < 8 or
                ...
                messages.error(request, "Please enter a strong password (minimum 8 characters, atleast one uppercase letter and one digit).")
                return redirect('resetPassword')
            else:
...
#       if order.coupon:
#     #     for c in coupon:
#         coupons = Coupon.objects.exclude(coupon_code=order.coupon)      
#       else:
#         coupons = Coupon.objects.all()
...
def coupon_list(request):
    ...
    for order in orders:
        if order.coupon:
            coupons = coupons.exclude(coupon_code=order.coupon)
